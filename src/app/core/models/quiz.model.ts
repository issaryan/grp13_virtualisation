...
export interface Question {
  ...
  correctOptionId?: string;   // id de la bonne option (optionnel)
  points?: number;            // points attribués à la question (optionnel)
  timeLimit?: number;         // en secondes (facultatif, limite pour répondre)
  selectedAnswer?: string;    // id de l’option choisie par l’étudiant (optionnel)
}
...
export interface Quiz {
  ...
  description: string;
  questions: Question[];
  createdBy: string;         // ID de l’enseignant
  createdAt: Date;
  updatedAt: Date;
  isPublished: boolean;
  timeLimit?: number;        // en minutes (facultatif, durée totale du quiz)
}
